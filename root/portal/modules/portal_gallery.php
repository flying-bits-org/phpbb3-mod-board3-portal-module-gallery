...
class portal_gallery_module
{
	...
	* Moved to new file
	function get_template_side($module_id)
	{
		...
		$block = new phpbb_gallery_block();
		$block->set_mode((int) $config['board3_gallery_small_mode_' . $module_id]);
		$block->set_display((int) $config['board3_gallery_small_display_' . $module_id]);
		$block->set_nums(array(
			'rows'		=> (int) $config['board3_gallery_small_rows_' . $module_id],
			'columns'	=> 1,
			'comments'	=> 0,
			'contests'	=> 0,
		));
		$block->set_pegas((bool) $config['board3_gallery_small_pgalleries_' . $module_id]);
		...
		if ($side_column)
		{
			return array(
				...
				'vars'	=> array(
					...
					'board3_gallery_small_display_' . $module_id		=> array('lang' => 'RRC_DISPLAY_OPTIONS',	'validate' => 'int',	'type' => 'custom',			'explain' => false,	'method' => 'rrc_display', 'submit' => 'store_rrc'),
	...
	*/
		return array(
			...
			'vars'	=> array(
				...
				'board3_gallery_center_display_' . $module_id		=> array('lang' => 'RRC_DISPLAY_OPTIONS',	'validate' => 'int',	'type' => 'custom',			'explain' => false,	'method' => 'rrc_display', 'submit' => 'store_rrc'),
	...
	*/
	function install($module_id)
	{
		...
		set_config('board3_gallery_center_mode_' . $module_id, 3);
		set_config('board3_gallery_center_display_' . $module_id, 5);
		set_config('board3_gallery_center_pgalleries_' . $module_id, 0);
		set_config('board3_gallery_center_comments_' . $module_id, 1);
		set_config('board3_gallery_version', '2.1.1');
